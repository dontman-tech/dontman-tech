...
- **🎙️ ARIA** — A voice-enabled assistant (Python brain + Kotlin Android bridge) with a skill router that scores speech against regex and keyword patterns before ever reaching for an LLM. 14 modular skills, 4 LLM backends, installable as a PWA, and a kill switch that halts everything at once. Now ships as a pip-installable package (`aria` / `aria-web` entry points) with a complete voice pipeline — STT, TTS replies, wake-word detection with partial-result matching, and mic self-hearing guards.
- **📱 Phoney** — A Linux companion for Android via the KDE Connect protocol: LAN discovery over UDP, TLS pairing with certificate trust, and the full Link-to-Windows feature set — battery status, remote mouse/keyboard, media control, find-my-phone, SMS, call alerts, and scrcpy screen mirroring (the piece KDE Connect lacks). Verified end-to-end pairing on a loopback protocol test, zero-dependency web dashboard, and a tagged v0.1.0 release.
